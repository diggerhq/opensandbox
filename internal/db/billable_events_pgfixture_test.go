...
// seedOrgWithBillingMode inserts an org with explicit plan, billing_mode,
// and optional Stripe customer ID — used by the sender's filtering
// tests so we can exercise every branch of the JOIN+WHERE clause.
func seedOrgWithBillingMode(t *testing.T, store *Store, plan, billingMode string, stripeCustomerID *string) uuid.UUID {
	t.Helper()
	ctx := context.Background()
	orgID := uuid.New()
	_, err := store.pool.Exec(ctx, `
		INSERT INTO orgs (id, name, slug, plan, billing_mode, stripe_customer_id, max_concurrent_sandboxes, max_sandbox_timeout_sec, max_disk_mb, max_memory_gb)
		VALUES ($1, $2, $3, $4, $5, $6, 100, 86400, 20480, 64)
	`, orgID, "sender-"+orgID.String()[:8], "sender-"+orgID.String()[:8], plan, billingMode, stripeCustomerID)
	if err != nil {
		t.Fatalf("seed org plan=%s billing_mode=%s: %v", plan, billingMode, err)
	}
	return orgID
}

func TestSender_listFiltersCorrectly_pgfixture(t *testing.T) {
	ctx := context.Background()
	store := openPgStore(t)

	// Three orgs covering all branches of the sender's JOIN+WHERE filter.
	// Each gets a distinct stripe_customer_id (unique constraint on orgs).
	cusUnified := "cus_test_unified_" + uuid.New().String()[:8]
	cusLegacy := "cus_test_legacy_" + uuid.New().String()[:8]
	orgUnified := seedOrgWithBillingMode(t, store, "pro", "unified", &cusUnified)
	orgLegacy := seedOrgWithBillingMode(t, store, "pro", "legacy", &cusLegacy)
	orgUnifiedNoCustomer := seedOrgWithBillingMode(t, store, "pro", "unified", nil)

	// Use far-future buckets to avoid cross-test interference.
	base := time.Date(2032, 1, 1, 12, 0, 0, 0, time.UTC)
	for i, orgID := range []uuid.UUID{orgUnified, orgLegacy, orgUnifiedNoCustomer} {
		ev := makeBucketEvent(orgID, BillableEventOverageUsage, 8192, 7200, base, i)
		if _, err := store.UpsertBillableEvent(ctx, ev); err != nil {
			t.Fatalf("upsert org %d: %v", i, err)
		}
	}

	pending, err := store.ListPendingBillableEventsForSender(ctx, 100)
	if err != nil {
		t.Fatalf("list pending: %v", err)
	}
	...
	for _, p := range pending {
		mine[p.Event.OrgID] = true
		if p.Event.OrgID == orgUnified && p.StripeCustomerID != cusUnified {
			t.Errorf("expected stripe_customer_id=%q, got %q", cusUnified, p.StripeCustomerID)
		}
	}
	if !mine[orgUnified] {
		t.Errorf("expected unified+customer org in result")
	}
	if mine[orgLegacy] {
		t.Errorf("legacy org must be excluded (UsageReporter handles it)")
	}
	if mine[orgUnifiedNoCustomer] {
		t.Errorf("unified org without stripe_customer_id must be excluded")
	}
}

func TestSender_markSent_pgfixture(t *testing.T) {
	ctx := context.Background()
	store := openPgStore(t)

	customerID := "cus_test_marksent"
	orgID := seedOrgWithBillingMode(t, store, "pro", "unified", &customerID)
	base := time.Date(2032, 2, 1, 12, 0, 0, 0, time.UTC)
	ev := makeBucketEvent(orgID, BillableEventOverageUsage, 8192, 7200, base, 0)
	if _, err := store.UpsertBillableEvent(ctx, ev); err != nil {
		t.Fatalf("upsert: %v", err)
	}

	// Find the event we just inserted (id is generated by the DB).
	pending, err := store.ListPendingBillableEventsForSender(ctx, 100)
	if err != nil {
		t.Fatalf("list: %v", err)
	}
	...
	for _, p := range pending {
		if p.Event.OrgID == orgID {
			eventID = p.Event.ID
			break
		}
	}
	if eventID == uuid.Nil {
		t.Fatalf("could not locate inserted event")
	}

	if err := store.MarkBillableEventSent(ctx, eventID, "stripe_test_event_42"); err != nil {
		t.Fatalf("mark sent: %v", err)
	}

	// Confirm row flipped + sender no longer sees it.
	rows, err := store.ListBillableEventsForBucket(ctx, orgID, ev.BucketStart)
	if err != nil {
	...
	if rows[0].DeliveryState != BillableDeliverySent {
		t.Errorf("delivery_state: got %q, want sent", rows[0].DeliveryState)
	}
	if rows[0].StripeEventID == nil || *rows[0].StripeEventID != "stripe_test_event_42" {
		t.Errorf("stripe_event_id mismatch: %v", rows[0].StripeEventID)
	}
	if rows[0].DeliveredAt == nil {
		t.Errorf("delivered_at should be set")
	}

	pending2, err := store.ListPendingBillableEventsForSender(ctx, 100)
	if err != nil {
		t.Fatalf("list pending 2: %v", err)
	}
	for _, p := range pending2 {
		if p.Event.ID == eventID {
			t.Errorf("sender still sees event after mark sent")
		}
	}
}

func TestAllocatorCandidates_filtersByBillingMode_pgfixture(t *testing.T) {
	ctx := context.Background()
	store := openPgStore(t)

	cusUnified := "cus_test_alloc_unified_" + uuid.New().String()[:8]
	cusLegacy := "cus_test_alloc_legacy_" + uuid.New().String()[:8]
	orgUnified := seedOrgWithBillingMode(t, store, "pro", "unified", &cusUnified)
	orgLegacy := seedOrgWithBillingMode(t, store, "pro", "legacy", &cusLegacy)
	orgFree := seedOrgWithBillingMode(t, store, "free", "unified", nil)

	// Far-future window so other tests don't interfere.
	bucket := time.Date(2032, 3, 1, 12, 0, 0, 0, time.UTC)
	end := bucket.Add(15 * time.Minute)
	cutoff := end
	lookbackStart := bucket.Add(-1 * time.Hour)

	// Each org gets one scale event covering the bucket.
	for _, orgID := range []uuid.UUID{orgUnified, orgLegacy, orgFree} {
		if _, err := store.pool.Exec(ctx,
			`INSERT INTO sandbox_scale_events (sandbox_id, org_id, memory_mb, cpu_percent, disk_mb, started_at, ended_at)
			 VALUES ($1, $2, $3, $4, $5, $6, $7)`,
			"sbx-"+orgID.String()[:8], orgID, 8192, 200, 20480, bucket, &end); err != nil {
			t.Fatalf("seed scale event: %v", err)
		}
	}
	...
	mine := map[uuid.UUID]bool{}
	for _, c := range cands {
		if c.BucketStart.Equal(bucket) {
			mine[c.OrgID] = true
		}
	}
	if !mine[orgUnified] {
		t.Errorf("expected pro+unified org as candidate")
	}
	if mine[orgLegacy] {
		t.Errorf("legacy pro org must NOT be a candidate (UsageReporter handles it)")
	}
	if mine[orgFree] {
		t.Errorf("free org must NOT be a candidate (legacy credit deduction handles it)")
	}
}
