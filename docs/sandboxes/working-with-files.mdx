...
<Note>
  All file I/O — including signed URL uploads/downloads — is streamed through the full stack (SDK → control plane → worker → VM). There is no separate "large file mode" to enable.
</Note>
