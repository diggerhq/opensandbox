...
import { Sandbox } from "@anthropic-ai/opencomputer";
...
<Note>
  `cpuCount` and `memoryMB` are not available in the Python SDK. Use the [HTTP API](/reference/api#sandbox-lifecycle) directly for custom resources from Python.
</Note>
