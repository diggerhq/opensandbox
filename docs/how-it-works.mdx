...
Your code talks to a **control plane** API, which provisions a virtual machine on a worker node. Commands, file operations, and agent sessions run inside that VM — events and I/O stream back over WebSocket. The control plane transparently proxies data-plane requests to the correct worker.
