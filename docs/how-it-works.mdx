...
Templates are built from Dockerfiles, but sandboxes are not containers. The Docker build produces a filesystem image that becomes the VM's root disk. The default template includes Ubuntu, Python 3, Node.js, and common CLI tools.

Custom templates skip repeated setup:

```typescript
const sandbox = await Sandbox.create({ template: "my-custom-stack" });
```

See [Templates](/sandboxes/templates) for building your own.
